document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderScoreboard(isEditable) {
        ...
        for (let i = 0; i < half; i++) {
            ...
            const colorContentL = isEditable 
                ? `<span class="score-cell color-col color-editable" data-index="${i}" data-field="color">${teamColorL}</span>`
                : `<span class="score-cell color-col"><div class="color-preview" style="background-color: ${teamColorL};"></div></span>`;
            ...
            if (rightIndex < TOTAL_TEAMS) {
                ...
                const colorContentR = isEditable 
                    ? `<span class="score-cell color-col color-editable" data-index="${rightIndex}" data-field="color">${teamColorR}</span>`
                    : `<span class="score-cell color-col"><div class="color-preview" style="background-color: ${teamColorR};"></div></span>`;
                ...
                `;
                 ...
                 // 25. takım için boş satır simetriyi korur (4 sütunlu boşluk bırakmak gerekiyor)
                 row.innerHTML += `
                    <span class="score-cell rank-col"></span>
                    <span class="score-cell team-col"></span>
                    <span class="score-cell color-col color-hidden"></span> 
        ...
        if (isEditable) {
            ...
            // Renk düzenleme hücrelerinin içeriğini HEX koduna ayarla
            document.querySelectorAll('.color-editable').forEach(cell => {
    ...
    // =======================================================
    // Bu kısım, herkesin verileri canlı görmesini sağlar.
    ...
    saveButton.addEventListener('click', () => {
        ...
        document.querySelectorAll('.score-row').forEach(row => {
            // Sadece düzenleme modunda olan (contenteditable) hücreleri al
